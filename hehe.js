...
app.get('/', (req, res) => {
    ...
    res.send(`<!DOCTYPE html><html><head><title>LVT Panel</title><style>
        ...
        .grid{display:grid;grid-template-columns:1fr 1fr;gap:20px;} button{cursor:pointer;background:#007bff;color:#fff;border:none;padding:8px 15px;border-radius:4px;}
    </style></head><body>
        ...
        <div class="grid">
            <div class="card"><h2>1. Tạo Key Mới</h2>
                <form action="/admin/create" method="POST">
                    ...
                    Máy: <input type="number" name="maxDevices" value="1" style="width:40px;">
                    <button type="submit">TẠO KEY</button>
                </form>
            </div>
            <div class="card"><h2>2. Bọc Script Bảo Mật</h2>
                ...
                <textarea id="b-final" readonly style="width:90%;height:40px;background:#eee;margin-top:5px;"></textarea>
            </div>
        </div>
        <div class="card"><h2>3. Danh sách Quản lý Key</h2>
            <form action="/admin/delete-bulk" method="POST"><select name="deleteType"><option value="expired">Xóa hết hạn</option><option value="all">Xóa tất cả</option></select><button type="submit" style="background:#dc3545">Xác nhận xóa</button></form><br>
