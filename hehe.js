...
const fs = require('fs');
const app = express();
const port = process.env.PORT || 3000; // Fix cho Render
...
app.get('/login', (req, res) => {
    ...
    <style>
        body { margin: 0; padding: 0; font-family: sans-serif; background: url('https://images.unsplash.com/photo-1550684848-fac1c5b4e853?q=80&w=2070&auto=format&fit=crop') center/cover no-repeat; height: 100vh; display: flex; align-items: center; justify-content: center; }
        .login-card { background: rgba(255, 255, 255, 0.1); backdrop-filter: blur(15px); padding: 40px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.2); width: 320px; text-align: center; color: #fff; }
        input { width: 100%; padding: 15px; margin: 20px 0; background: rgba(0,0,0,0.3); border: 1px solid #0ff; border-radius: 30px; color: #0ff; text-align: center; outline: none; box-sizing: border-box;}
...
app.post('/api/ai', async (req, res) => {
    try {
        ...
        res.json({ status: 'success', text: d?.candidates?.[0]?.content?.parts?.[0]?.text || "Lỗi AI" });
...
// Giao diện chính (Hồi phục 100%)
app.get('/', (req, res) => {
    let db = loadDB(); let keysHtml = '';
    for (let k in db) {
        let kd = db[k]; let isVip = k.startsWith('VIP-');
        let expT = kd.exp === 'pending' ? 'Chờ kích hoạt' : (kd.exp === 'permanent' ? 'Vĩnh viễn' : new Date(kd.exp).toLocaleString());
        keysHtml += `<tr style="${isVip ? 'background:#fffbf0;' : ''}">
            ...
            <td><a href="/admin/reset-time/${k}">[Reset TG]</a> | <a href="/admin/delete/${k}">[Xóa]</a></td></tr>`;
    }
    res.send(`<!DOCTYPE html><html><head><title>LVT Panel</title><style>
        ...
        <div class="grid">
            ...
            <div class="card"><h2>2. Bọc Script</h2>
                <input type="text" id="b-url" placeholder="Match URL" value="*://*.olm.vn/*" style="width:90%">
                <textarea id="b-code" placeholder="Dán code gốc vào đây..." style="width:90%;height:60px;"></textarea>
                <button onclick="build()">BỌC NGAY</button>
                <textarea id="b-final" readonly style="width:90%;height:40px;background:#eee;"></textarea>
            </div>
        </div>
        <div class="card"><h2>3. Quản lý Key</h2>
        ...
        <script>
            function build() {
                let url = document.getElementById('b-url').value; let code = document.getElementById('b-code').value;
                if(!code) return alert('Dán code!');
                let base64 = btoa(unescape(encodeURIComponent(code)));
                document.getElementById('b-final').value = "// ==UserScript==\\n// @name Protected\\n// @match " + url + "\\n// ==/UserScript==\\n(function(){ eval(decodeURIComponent(escape(atob('" + base64 + "')))); })();";
            }
        </script>
    </body></html>`);
});
